...
long flat_index(dvec grind, hydro_problem* hp) {
	...
	for(ax = 0; ax < hp->dim; ++ax)
		flatind += hp->strides.ijk[ax]*(STENCIL_SIZE+grind.ijk[ax]);
	return flatind; 
}
...
hydro_derived_state get_derived_state(dvec cell, int iterax, hydro_problem* hp) {
	...
	ds.rho_l = hvec.rho-6*hvec.com.xyz[iterax]/(hp->dx);
	ds.rho_c = hvec.rho;
	ds.rho_r = hvec.rho+6*hvec.com.xyz[iterax]/(hp->dx);
	...
	for(ax = 0, v2 = 0.0; ax < hp->dim; ++ax) {
		ds.v.xyz[ax] = hvec.mom.xyz[ax]/hvec.rho;
		v2 += ds.v.xyz[ax]*ds.v.xyz[ax];	
	}
...
#define get_cell_char_speed(grind, hp) {	\
	for(ax = 0; ax < hp->dim; ++ax) {	\
		...
		if(fabs(ds.v.xyz[ax] + ds.c_l) > alpha_max) alpha_max = fabs(ds.v.xyz[ax] + ds.c_l); 	\
		if(fabs(ds.v.xyz[ax] - ds.c_l) > alpha_max) alpha_max = fabs(ds.v.xyz[ax] - ds.c_l);	\
		if(fabs(ds.v.xyz[ax] + ds.c_r) > alpha_max) alpha_max = fabs(ds.v.xyz[ax] + ds.c_r); 	\
		if(fabs(ds.v.xyz[ax] - ds.c_r) > alpha_max) alpha_max = fabs(ds.v.xyz[ax] - ds.c_r);	\
	}	\
}
	...
	alpha_max = 0.0;
	if(hp->dim == 1) {
		for(grind.i = 0; grind.i < hp->nx.i; ++grind.i)
			get_cell_char_speed(grind, hp);
	}
	else if(hp->dim == 2) {
		for(grind.i = 0; grind.i < hp->nx.i; ++grind.i)
		for(grind.j = 0; grind.j < hp->nx.j; ++grind.j)
			get_cell_char_speed(grind, hp);
	}
	else if(hp->dim == 3) {
		for(grind.i = 0; grind.i < hp->nx.i; ++grind.i)
		for(grind.j = 0; grind.j < hp->nx.j; ++grind.j)
		for(grind.k = 0; grind.k < hp->nx.k; ++grind.k)
			get_cell_char_speed(grind, hp);
	}
	return alpha_max;
}
...
#define update_cell_source_terms(grind, hp) {	\
	...
	cell = &hp->grid[flatind];	\
	for(ax = 0; ax < hp->dim; ++ax)	\
		cell->com.xyz[ax] += dt*cell->mom.xyz[ax];	\
}
...
hydro_vector flux_solve_hll(hydro_derived_state left, 
	...
	if(left.c_r - left.v.xyz[skax] > left_alpha) left_alpha = left.c_r - left.v.xyz[skax];
	if(right.c_l - right.v.xyz[skax] > left_alpha) left_alpha = right.c_l - right.v.xyz[skax];
	right_alpha = 0.0; // right wave speed
	if(right.v.xyz[skax] + right.c_l > right_alpha) right_alpha = right.v.xyz[skax] + right.c_l;
	if(left.v.xyz[skax] + left.c_r > right_alpha) right_alpha = left.v.xyz[skax] + left.c_r;

	// Compute HLL fluxes
	flux.rho = (right_alpha*left.rho_r*left.v.xyz[skax] + left_alpha*right.rho_l*right.v.xyz[skax] 
			- left_alpha*right_alpha*(right.rho_l - left.rho_r))/(right_alpha + left_alpha); 
	for(ax = 0; ax < hp->dim; ++ax) {
		flux.mom.xyz[ax] = (right_alpha*(left.rho_r*left.v.xyz[ax]*left.v.xyz[skax] + (ax==skax)*left.p_r) + left_alpha*(right.rho_l*right.v.xyz[ax]*right.v.xyz[skax] + (ax==skax)*right.p_l) 
			- left_alpha*right_alpha*(right.rho_l*right.v.xyz[ax] - left.rho_r*left.v.xyz[ax]))/(right_alpha + left_alpha); 
	}
	flux.etot = (right_alpha*(left.etot + left.p_r)*left.v.xyz[skax] + left_alpha*(right.etot + right.p_l)*right.v.xyz[skax]
...
#define minmod(x, y, z) (0.25*abs(sgn(x)+sgn(y))*(sgn(x)+sgn(z))*min3(abs(x), abs(y), abs(z)))
	...
	for(ax = 0; ax < hp->dim; ++ax) {
		c_l = dsl->v.xyz[ax] + 0.5*minmod(theta*(dsl->v.xyz[ax]-dsll->v.xyz[ax]), 0.5*(dsr->v.xyz[ax]-dsll->v.xyz[ax]), theta*(dsr->v.xyz[ax]-dsl->v.xyz[ax]));
		c_r = dsr->v.xyz[ax] - 0.5*minmod(theta*(dsr->v.xyz[ax]-dsl->v.xyz[ax]), 0.5*(dsrr->v.xyz[ax]-dsl->v.xyz[ax]), theta*(dsrr->v.xyz[ax]-dsr->v.xyz[ax]));
		dsl->v.xyz[ax] = c_l;
		dsr->v.xyz[ax] = c_r;
	}
	...
	for(ax = 0; ax < hp->dim; ++ax) {
		grad_l = 2.0*(dsl->v.xyz[ax]-dsl->v.xyz[ax]);
		grad_r = 2.0*(dsr->v.xyz[ax]-dsr->v.xyz[ax]);
		grad_max = -(dsr->v.xyz[ax] - dsl->v.xyz[ax]);
		if(grad_l < grad_max)
			dsl->v.xyz[ax] = dsl->v.xyz[ax]+0.5*grad_max;
		if(grad_r > grad_max)
			dsr->v.xyz[ax] = dsr->v.xyz[ax]-0.5*grad_max;
	}
...
void update_skewer(dvec skewer, int iterax, real fluxfac, hydro_problem* hp) {
	...
	int i, ax;
	long flatind;
	hydro_derived_state state_skewer[hp->nx.ijk[iterax]+2*STENCIL_SIZE];
	for(i = 0; i < hp->nx.ijk[iterax]; ++i) {
		skewer.ijk[iterax] = i;
	...
	for(i = 0; i < STENCIL_SIZE; ++i) {
		...
		state_skewer[ZL(-i)].v.xyz[iterax] *= -1;
		...
		state_skewer[ZR(hp->nx.ijk[iterax]+i)] = state_skewer[ZL(hp->nx.ijk[iterax]-i)];
		state_skewer[ZR(hp->nx.ijk[iterax]+i)].v.xyz[iterax] *= -1;

		//tmp = state_skewer[ZR(hp->nx.ijk[iterax]+i)].rho_r;
		//state_skewer[ZR(hp->nx.ijk[iterax]+i)].rho_r = state_skewer[ZR(hp->nx.ijk[iterax]+i)].rho_l;
		//state_skewer[ZR(hp->nx.ijk[iterax]+i)].rho_l = tmp; 

	}
	else if(hp->bctype == BC_PERIODIC) 
	for(i = 0; i < STENCIL_SIZE; ++i) {
		state_skewer[ZL(-i)] = state_skewer[ZL(hp->nx.ijk[iterax]-i)];
		state_skewer[ZR(hp->nx.ijk[iterax]+i)] = state_skewer[ZR(i)];
	}
	...
	for(i = 0; i <= hp->nx.ijk[iterax]; ++i) {
		...
		skewer.ijk[iterax] = i-1;
		flatind = flat_index(skewer, hp);
		hp->grid[flatind].rho -= fluxfac*flux.rho;
		for(ax = 0; ax < hp->dim; ++ax)
			hp->grid[flatind].mom.xyz[ax] -= fluxfac*flux.mom.xyz[ax];
		hp->grid[flatind].etot -= fluxfac*flux.etot;
		hp->grid[flatind].com.xyz[iterax] -= 0.5*hp->dx*fluxfac*flux.rho;
...
void writeout(hydro_problem* hp) {
	...
	fwrite(hp->grid+STENCIL_SIZE, sizeof(hydro_vector), hp->nx.i, output);
...
void update_lagrange_mesh(real dt, hydro_problem* hp) {
	...
	for(v = 0; v < hp->nverts; ++v) {
		if(hp->mesh_verts[v].pos.xyz[ax] != 0.0 && hp->mesh_verts[v].pos.xyz[ax] != 1.0)
			hp->mesh_verts[v].pos.xyz[ax] += dt*hp->mesh_verts[v].vel.xyz[ax];
	}
	...
	for(iter = 0; max_err > FUZZ && iter < 1000; ++iter) {
		...
		for(t = 0; t < hp->ntets; ++t) {
			...
			for(v = 0; v < hp->dim+1; ++v)
				for(i = 0; i < hp->dim; ++i)
					dv[v].xyz[i] = hp->mesh_verts[tet.verts[v]].pos.xyz[i];
			vol = 0.5*((dv[1].x-dv[0].x)*(dv[2].y-dv[0].y)-(dv[2].x-dv[0].x)*(dv[1].y-dv[0].y));
		...
		for(ax = 0; ax < 2; ++ax) {
			...
			for(t = 0; t < hp->ntets; ++t) {
				...
				for(v = 0; v < hp->dim+1; ++v)
					for(i = 0; i < hp->dim; ++i)
						dv[v][i] = hp->mesh_verts[tet.verts[v]].pos.xyz[i];
				...
				for(v = 0, mtest = 0.0; v < hp->dim+1; ++v) {
					...
					sumx += hp->mesh_verts[tet.verts[v]].pos.xyz[ax];
					mtest += hp->mesh_verts[tet.verts[v]].rho*hp->mesh_verts[tet.verts[v]].pos.xyz[ax];
				}
				mtest = (vol/12.0)*(sumrho*sumx+mtest);
				err = tet.com.xyz[ax] - mtest;
				if(fabs(err/tet.mass) > max_err) 
					max_err = fabs(err/(tet.mass+FUZZ));
			...
			for(v = 0; v < hp->nverts; ++v) {
				drho[v] /= tperv[v];
				if(hp->mesh_verts[v].pos.xyz[ax] != 0.0 && hp->mesh_verts[v].pos.xyz[ax] != 1.0)
					hp->mesh_verts[v].pos.xyz[ax] += 0.0001*drho[v];
			}
		...
		for(ax = 0; ax < 2; ++ax) {
			...
			for(t = 0; t < hp->ntets; ++t) {
				...
				for(v = 0; v < hp->dim+1; ++v)
					for(i = 0; i < hp->dim; ++i)
						dv[v][i] = hp->mesh_verts[tet.verts[v]].vel.xyz[i];
				vol = 0.5*((dv[1][0]-dv[0][0])*(dv[2][1]-dv[0][1])-(dv[2][0]-dv[0][0])*(dv[1][1]-dv[0][1]));
				real vfac = vol/(hp->dim+1);
				...
				for(v = 0, mtest = 0.0; v < hp->dim+1; ++v) {
					sumrho += hp->mesh_verts[tet.verts[v]].rho;
					sumx += hp->mesh_verts[tet.verts[v]].vel.xyz[ax];
					mtest += hp->mesh_verts[tet.verts[v]].rho*hp->mesh_verts[tet.verts[v]].vel.xyz[ax];
				}
				mtest = (vol/12.0)*(sumrho*sumx+mtest);
				err = tet.mom.xyz[ax] - mtest;
				if(fabs(err/tet.mass) > max_err) 
					max_err = fabs(err/(tet.mass+FUZZ));
			...
			for(v = 0; v < hp->nverts; ++v) {
				drho[v] /= tperv[v];
				//if(hp->mesh_verts[v].pos.xyz[ax] != 0.0 && hp->mesh_verts[v].pos.xyz[ax] != 1.0)
					//hp->mesh_verts[v].vel.xyz[ax] += 0.001*drho[v];
			}
	...
	for(t = 0; t < hp->ntets; ++t) {
		for(i = 0; i < 3; ++i)
			hp->mesh_tets[t].com.xyz[i] += dt*hp->mesh_tets[t].mom.xyz[i];
	}
...
int init(hydro_problem* hp) {
	...
	for(t = 0; t < hp->ntets; ++t) {
		...
		for(i = 0; i < hp->dim; ++i) {
			...
			for(v = 0; v < hp->dim+1; ++v) {
				sumrho += hp->mesh_verts[tet.verts[v]].rho;
				sumx += hp->mesh_verts[tet.verts[v]].pos.xyz[i];
				tet.com.xyz[i] += hp->mesh_verts[tet.verts[v]].rho*hp->mesh_verts[tet.verts[v]].pos.xyz[i];
			}
			tet.com.xyz[i] = (vol/12.0)*(sumrho*sumx+tet.com.xyz[i]);

			// Cell momentum
			sumrho = 0.0, sumx = 0.0;
			for(v = 0; v < hp->dim+1; ++v) {
				sumrho += hp->mesh_verts[tet.verts[v]].rho;
				sumx += hp->mesh_verts[tet.verts[v]].vel.xyz[i];
				tet.mom.xyz[i] += hp->mesh_verts[tet.verts[v]].rho*hp->mesh_verts[tet.verts[v]].vel.xyz[i];
			}
			tet.mom.xyz[i] = (vol/12.0)*(sumrho*sumx+tet.mom.xyz[i]);
